...
class Card(Component):
    props = {
        ...
        'variant': Prop(str, default="default", choices=["default", "bordered"]),
        'js': Prop(str, default="none", choices=["none", "alpine", "htmx"])
    }
    static_bundle = "components/card.bundle.css"

    def __init__(self, **kwargs):
        djuno_file = os.path.join(
            settings.BASE_DIR, 'components', 'card', 'card.djuno')
    ...
    def get_context_data(self):
        ...
        if self.kwargs['js'] == "alpine":
            js_attrs = 'x-data="card"'
        elif self.kwargs['js'] == "htmx":
            js_attrs = 'hx-post="/refresh/" hx-swap="outerHTML"'
        return {
            **super().get_context_data(),
            'class': self.kwargs['variant'],
            'js_attrs': js_attrs
        }
