...
class Button(Component):
    props = {
        ...
        'variant': Prop(str, default="primary", choices=["primary", "secondary"]),
        'size': Prop(str, default="lg", choices=["sm", "lg"]),
        'disabled': Prop(bool, default=False),
        'js': Prop(str, default="none", choices=["none", "alpine", "htmx"])
    }
    static_bundle = "components/button.bundle.css"

    def __init__(self, **kwargs):
        djuno_file = os.path.join(
            settings.BASE_DIR, 'components', 'button', 'button.djuno')
    ...
    def get_context_data(self):
        ...
        if self.kwargs['js'] == "alpine":
            js_attrs = 'x-data="{ disabled: {{ disabled|lower }} }" x-on:click="disabled = !disabled"'
